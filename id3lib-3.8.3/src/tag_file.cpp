...
// gam ---
bool ShittyMoveFile(const String& source, const String& dest)
{
    ...
    if(fileSize)
    {
        ...
        if(fread(&fileBuffer[0], sizeof(BYTE), fileSize, sourceFile) != fileSize)
        {
            fclose(sourceFile);
            fclose(destFile);
            return(false);
        ...
        if(fwrite(&fileBuffer[0], sizeof(BYTE), fileSize, destFile) != fileSize)
        {
            fclose(sourceFile);
            fclose(destFile);
            return(false);
...
size_t RenderV2ToFile(const ID3_TagImpl& tag, fstream& file)
{
  ...
  else
  {
    ...
    if(stat(filename.c_str(), &fileStat) == 0)
    {
      ...
      // gam ---
#if defined(WIN32)
        // If we just call remove/rename here and the file is actually a hard link, 
        // the link will be broken and the a second (altered) copy of the file
        // will be written:
        ShittyMoveFile(sTempFile, filename);
#else      
